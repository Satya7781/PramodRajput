...
export async function GET(req: NextRequest) {
  ...
  try {
    const [
      ...
      certificates_generated, news_articles, photo_albums, videos, memories,
    ] = await Promise.all([
      ...
      queryOne<{ count: string }>(`SELECT COUNT(*) as count FROM event_memories WHERE status='published'`),
    ]);
    ...
    return ok({
      ...
      memories:               parseInt(memories?.count               ?? '0', 10),
    });
  } catch (e) {
