...
import { auditLogs as auditLogsApi } from '@/lib/api-client';
...
export default function AuditLogsPage() {
  ...
  const filtered = logs.filter((l) => {
    ...
    return l.action.toLowerCase().includes(s) || (l.entity_type ?? '').toLowerCase().includes(s) || (l.user_name ?? '').toLowerCase().includes(s);
  });

  const ACTION_COLOR: Record<string, string> = {
    create: 'bg-green-100 text-green-700', update: 'bg-primary/10 text-primary',
    delete: 'bg-red-100 text-red-700', login: 'bg-muted text-muted-foreground',
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-3">
          <Input placeholder="Search action, entity, user..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-56" />
                ...
                {filtered.map((log) => (
                  <tr key={log.id} className="hover:bg-muted/30 transition-colors">
                    <td className="px-4 py-3">
                      <span className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-medium ${getColor(log.action)}`}>{log.action}</span>
                    </td>
                    <td className="px-4 py-3 text-sm text-muted-foreground hidden md:table-cell">
                      {log.entity_type ?? '—'}
                      {log.entity_id && <span className="ml-1 text-xs font-mono opacity-60">{log.entity_id.slice(0, 8)}…</span>}
                    </td>
                    <td className="px-4 py-3 text-sm text-muted-foreground hidden sm:table-cell">{log.user_name ?? 'System'}</td>
                    <td className="px-4 py-3 text-sm text-muted-foreground">{formatDateTime(log.created_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
