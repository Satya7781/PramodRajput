        ...
        <div className="container mx-auto px-4 lg:px-8">
          <div className="max-w-3xl animate-slide-up">
            ...
            <p className="text-lg text-muted-foreground leading-relaxed">
              Join us at upcoming events or explore past initiatives. Every gathering is an opportunity to connect, learn, and contribute to the community.
            </p>
          </div>
        </div>
      </section>
          ...
          {sections.map((section) => (
            <div key={section.label}>
              <div className="flex items-center gap-3 mb-8">
                ...
                <span className="rounded-full bg-muted px-3 py-1 text-sm text-muted-foreground">{section.items.length}</span>
              </div>
              {section.items.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {section.items.map((event, i) => (
                    <Link key={event.id} href={`/events/${event.slug}`} className="group rounded-2xl border border-border bg-card overflow-hidden hover:shadow-xl transition-all animate-slide-up" style={{ animationDelay: `${i * 80}ms` }}>
                      <div className="aspect-[16/10] overflow-hidden bg-muted relative">
                        ...
                        {event.status === 'registration_open' && (
                          <span className="absolute top-3 right-3 rounded-full bg-secondary text-secondary-foreground px-3 py-1 text-xs font-medium">Registration Open</span>
                        )}
                      </div>
                      <div className="p-6">
                        ...
                        <p className="text-sm text-muted-foreground line-clamp-2 mb-4">{event.short_description}</p>
                        {event.max_participants && (
                          <div className="flex items-center gap-1 text-xs text-muted-foreground mb-4">
                            <Users className="h-3 w-3" /> Max {event.max_participants} participants
                          </div>
                        )}
