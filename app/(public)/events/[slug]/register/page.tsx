...
async function getEventData(slug: string) {
  ...
  if (!form) return { event: event as Event, form: null, fields: [] };
...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  return {
    title: `Register for ${data.event.title} — Pramod Rajput`,
    description: `Register for ${data.event.title}`,
  };
}
            ...
            {regEnded ? (
              ...
              <div className="rounded-2xl border border-border bg-card p-8 text-center">
                ...
                <p className="text-muted-foreground">
                  Registration opens on {event.registration_start ? formatDate(event.registration_start) : 'soon'}.
                </p>
              </div>
