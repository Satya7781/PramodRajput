...
async function getNewsData() {
  ...
  return {
    ...
    categories: (categoriesRes.data || []) as NewsCategory[],
  };
}
        ...
        <div className="container mx-auto px-4 lg:px-8">
          <div className="max-w-3xl animate-slide-up">
            ...
            <p className="text-lg text-muted-foreground leading-relaxed">
              Stay informed about the latest initiatives, events, and community stories.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 lg:px-8">
          ...
          {featured && (
            <Link
              ...
              className="group mb-12 grid grid-cols-1 lg:grid-cols-2 gap-8 rounded-2xl border border-border bg-card overflow-hidden hover:shadow-xl transition-all"
            >
              ...
              <div className="p-8 flex flex-col justify-center">
                {featured.news_categories && (
                  <span className="text-xs font-semibold text-primary uppercase tracking-wider mb-3">
                    Featured · {featured.news_categories.name}
                  </span>
                )}
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-8">
            ...
            <div className="flex flex-wrap gap-2">
              <Link
                href="/news"
                className="rounded-full bg-primary text-primary-foreground px-4 py-1.5 text-sm font-medium"
              >
                All
              </Link>
              {categories.map((cat) => (
                <Link
                  ...
                  className="rounded-full border border-border px-4 py-1.5 text-sm font-medium text-muted-foreground hover:bg-muted transition-colors"
                >
                  {cat.name}
                </Link>
              ))}
          ...
          {rest.length > 0 ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              {news.length === 0 ? 'No news articles published yet.' : null}
            </div>
          )}
        </div>
      </section>
