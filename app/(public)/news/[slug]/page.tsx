...
export default function NewsArticlePage() {
  ...
  if (notFound || !article) return (
    <div className="flex items-center justify-center min-h-[60vh] text-center px-4">
      <div><h1 className="text-2xl font-bold mb-2">{t('news','notFound')}</h1>
        <Link href="/news" className="text-primary underline text-sm">{t('news','backToNews')}</Link></div>
    </div>
  );
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3">
                {photos.map((p, i) => (
                  <button key={p.id} onClick={() => setLightbox({ open: true, index: i })}
                    ...
                    <img src={p.url} alt={p.caption ?? ''} className="h-full w-full object-cover group-hover:scale-105 transition-transform duration-400" loading="lazy" />
                ...
                {videos.map((v, i) => {
                  ...
                  return (
                    <div key={v.id} className="rounded-xl border border-border overflow-hidden bg-card">
                      ...
                      {embed.type === 'facebook' && (
                        <div className="aspect-video flex items-center justify-center bg-[#1877F2]/5">
                          <iframe
